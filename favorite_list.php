...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="./css/favorite.css?v=202006011338" />
...
<div class="pages">
    ...
    <div class="items">
        ...
        <div class="busCategory">
            <?
            $sql = "SELECT * FROM favorite WHERE user_id = '".$_SESSION['user_id']."'";
            $query = mysqli_query($conn, $sql);

            while($row = mysqli_fetch_array($query)) {
                ?>
                <p class="p_category">
                    <span name="busStopName"><?=$row["station_name"];?></span> <i class="fas fa-star" id="favorites" style="color: yellow;"></i>
                    <input type="hidden" name="busStopId" value="<?=$row["station_id"];?>"/>
                    <input type="hidden" name="busX" value="<?=$row["x"];?>"/>
                    <input type="hidden" name="busY" value="<?=$row["y"];?>"/>
                </p>
            <? } ?>
        </div>
